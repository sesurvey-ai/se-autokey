...
def run_import_xml(cfg, args):
    ...
    try:
        ...
        # ด่านเวลาสำรวจ — ขาดเมื่อไหร่ EMCS ปัดตกแน่นอน หยุดก่อนเปิด (ไม่ทิ้ง draft ค้าง)
        _tg = _survey_time_gate(data)
        if _tg:
            banner("หยุด: ไม่มีเวลาสำรวจ — กรอก EMCS ไม่ได้")
            log_plain(f"  {_tg}")
            driver.quit()
            return
...
def _survey_time_gate(data) -> str:
    """เวลาสำรวจว่าง = กรอก EMCS ไม่ได้ตั้งแต่ต้น → คืนข้อความเหตุผล ('' = ผ่าน)
    ...
    ⛔ **หยุดก่อนเปิด EMCS** (user เคาะ 01/09/69) ไม่ใช่เปิดไปแล้วค่อยหยุดถาม —
       ถ้าไปต่อจะได้ของแถมที่ไม่มีใครอยากได้: draft ค้างใน EMCS ที่ลบไม่ได้
       (ยกเลิกได้อย่างเดียว) และล็อกเรื่องนั้นไว้กับ username ที่บอทใช้

    เจอจริง 01/09/69 เคลม 2026013168056 — ISURVEY ว่างทั้ง 4 ช่อง บอทรู้ตั้งแต่ตอน
    อ่านข้อมูลแต่เดินหน้าต่อจนโดน validForm() ปัดตกเงียบ ๆ เสียเวลาไปทั้งรอบ
    แล้วคนต้องไปกรอกเองบน EMCS ทั้งเรื่อง
    """
    missing = emcs.missing_survey_times(data)
    if not missing:
        return ""
    nl = chr(10)
    return (
        "ISURVEY ยังไม่มี: " + ", ".join(missing) + nl
        + '  EMCS บังคับช่องเหล่านี้ และเป็นช่องวันที่/เวลา จึงใส่ "-" แทนไม่ได้' + nl
        + "  → เติมที่ ISURVEY ให้ครบก่อน แล้วรันใหม่" + nl
        + "  (จะกรอกเองบน EMCS ก็ได้ แต่บอทช่วยไม่ได้ตั้งแต่ช่องแรก)"
    )
