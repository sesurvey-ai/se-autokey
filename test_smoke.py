...
# _plate: ทะเบียนแบบที่ EMCS รับ — ลบช่องว่าง + ขีด/เครื่องหมาย + คำพ่วง (20/09/69 เคลม 2026013076932 "83-2668") · ตัวอย่างชุดเดียวกับ backend/web
check("_plate ลบช่องว่าง/ขีด/เครื่องหมาย/คำพ่วงในทะเบียน (สูตรเดียวกับ backend/web emcsPlate)",
      emcs._plate("9กฆ 5003") == "9กฆ5003" and emcs._plate(" กท 1234 ") == "กท1234"
      and emcs._plate("83-2668") == "832668" and emcs._plate("8กจ-5386") == "8กจ5386"
      and emcs._plate("6-7815 สภ.ศรีราชา(ป้ายแดง)") == "67815" and emcs._plate("743100(หัว),716951") == "743100"
      and emcs._plate("ข-8052 (ป้ายแดง)") == "ข8052" and emcs._plate("ก-0816/ปด") == "ก0816" and emcs._plate("2กฌ-4799 สก.") == "2กฌ4799"
      and emcs._plate("ก1006อุทัย ปด.") == "ก1006อุทัย" and emcs._plate("--") == "" and emcs._plate("00") == "00" and emcs._plate("ษข9066") == "ษข9066"
...
# เคส 21BR10AVD-6905-001860) แต่เส้นบอทกรอกตรงเคยอ่านคำอ่านดิบจาก report → ได้ 'ชั้น 2บวก'
...
with tempfile.TemporaryDirectory() as _xd2:
    ...
    check("ประเภทกรมธรรม์รถประกัน: อ่านจากระดับรายงาน ไม่ใช่บล็อกรถ",
          _pt["insured"].get("policy_type") == "ประเภท 2 บวก")
...
check("รหัส → คำอ่าน ครบ 8 ค่า",
      ...
      == ["ประเภท 1", "ประเภท 2", "ประเภท 3", "พรบ.", "ประเภท 5",
          "ไม่พบความคุ้มครอง", "ประเภท 2 บวก", "ประเภท 3 บวก"])
...
check("เว้นวรรคก่อน 'บวก' (ไม่ใช่ 'ประเภท 2บวก' ติดกัน)", _ptn("52") == "ประเภท 2 บวก")
...
# ประเภทรถบนเรื่องที่บันทึกแล้ว: ห้ามเปลี่ยน (EMCS ลบข้อมูลทิ้ง) แต่ต้องไม่เตือนผิด
# เมื่อค่าถูกอยู่แล้วแค่สะกดต่าง — ISURVEY 'เก๋งเอเซีย' vs EMCS 'เก๋งเอเชีย' (ซ/ช)
from rapidfuzz import fuzz as _fz_ct  # noqa: E402
check("ประเภทรถ: สะกดต่าง (ซ/ช) ต้องนับว่าเหมือนกัน",
      _fz_ct.WRatio("เก๋งเอเชีย", "เก๋งเอเซีย") >= emcs.CAR_TYPE_SAME_SCORE)
check("ประเภทรถ: ประเภทที่ต่างกันจริงต้องไม่หลุดผ่านเกณฑ์",
      all(_fz_ct.WRatio(a, b) < emcs.CAR_TYPE_SAME_SCORE for a, b in
          [("เก๋งเอเชีย", "เก๋งยุโรป"), ("เก๋งยุโรป", "เก๋งเอเซีย"),
           ("กระบะ", "รถบรรทุก"), ("รถตู้", "รถอื่นๆ")]))
...
# ข้อความ "ตรวจรูปแบบ" เป็นประโยค เอาไปหาช่องตรง ๆ ไม่เจอ แต่ชื่อช่องอยู่ในคำพูด
check("quoted: ดึงชื่อช่องจากเครื่องหมายคำพูดในประโยคได้",
      emcs._quoted_field_names(
          "1. กรณี [เคลมสด] ต้องระบุ 'วันที่เกิดเหตุและเวลา' 'วันที่ลูกค้าแจ้งบ.ประกัน' "
          "'วันที่สำรวจภัยเสร็จ' ให้อยู่ภายใน 24 ชั่วโมง.")
      == ["วันที่เกิดเหตุและเวลา", "วันที่ลูกค้าแจ้งบ.ประกัน", "วันที่สำรวจภัยเสร็จ"])
check("quoted: ไม่มีคำพูด → ลิสต์ว่าง (ไม่เดา)",
      emcs._quoted_field_names("1. สถานที่เกิดเหตุ") == []
      and emcs._quoted_field_names("") == [])
check("quoted: ชื่อซ้ำนับครั้งเดียว + ตัดช่องว่างเกิน",
      emcs._quoted_field_names("'วันที่  สำรวจ' และ 'วันที่ สำรวจ'") == ["วันที่ สำรวจ"])
...
# ---- 22f6. ถามก่อนกดบันทึก: EMCS บังคับช่องไหนที่ยังว่าง ----
# user เคาะ 02/09/69 — เดิมกดบันทึกแล้วรอโดนปัดตกถึงจะรู้ว่าขาดอะไร
# ⛔ ดอกจันแดงเชื่อไม่ได้: อ่าน vlidSurvey() ของ EMCS จริงพบว่าบังคับ 38 ช่อง
#    แต่มีอีก 3 ช่องที่มีดอกจันแล้วถูก comment ทิ้ง → กฎจริงมีที่เดียวคือ validForm()
_o_pre, _n_pre, _c_pre, _k_pre = _run_save_main(
    ["บันทึกข้อมูลเรียบร้อย S68426012345"], is_new=False,
    validform={"ok": False, "err": "", "control": "txtKm_No",
               "missing": ["หมายเลข กม.", "แขวง/ตำบล ที่เกิดเหตุ"], "alert": ""})
check("ถามก่อนกดบันทึก เมื่อ validForm บอกว่ายังขาด",
      _n_pre >= 1 and "หมายเลข กม." in _k_pre.get("label", ""),
      f"ถาม {_n_pre} ครั้ง · {_k_pre.get('label','')[:60]}")
check("ถามแล้วยังกดบันทึกต่อ (ไม่ยอมแพ้ตั้งแต่ยังไม่กด)", _c_pre >= 1, f"กด {_c_pre} ครั้ง")

# validForm ผ่าน → ห้ามถามให้เสียเวลา
_o_np, _n_np, _c_np, _ = _run_save_main(
    ["บันทึกข้อมูลเรียบร้อย S68426012345"], is_new=False,
    validform={"ok": True, "err": "", "control": "", "missing": [], "alert": ""})
check("ข้อมูลครบ → ไม่ถาม กดบันทึกเลย", _n_np == 0 and _c_np == 1,
      f"ถาม {_n_np} ครั้ง / กด {_c_np} ครั้ง")

# ⛔ ถามเฉพาะรอบแรก — รอบถัดไปมีเส้น "หยุดรอคนกรอก" ตอนโดนปัดตกอยู่แล้ว
_esrc2 = __import__("inspect").getsource(emcs.save_main_form)
check("ถามก่อนกดเฉพาะรอบแรก (ไม่ถามซ้ำทุกรอบ)", "if attempt == 1:" in _esrc2)
...
# ถาม 2 ครั้งได้: ครั้งแรกคือด่านถามก่อนกด (validForm บอกว่าขาด) ครั้งที่สองคือหลังโดนปัดตก
# — ของจริงคนเติมตั้งแต่ครั้งแรกแล้วบันทึกผ่านเลย ที่ห้ามคือ "กดซ้ำโดยไม่ถาม"
check("เงียบ + EMCS บอกช่องที่ขาด → ถามคน ไม่กดวนเดา",
...
check("ตั้งค่า EMCS: มีช่องชื่อผู้ใช้ + รหัสผ่าน",
      'id="emcsuser"' in _page and 'id="emcspass"' in _page)
# ⛔ EMCS ไม่มีปุ่มทดสอบล็อกอินโดยตั้งใจ — ทดสอบ = เปิดเข้าระบบบริษัทประกันจริง
#    ซึ่งมีกติกาว่าห้ามแตะโดยไม่จำเป็น (ISURVEY เป็นระบบของเราเอง ทดสอบได้)
check("ตั้งค่า EMCS: ⛔ ต้องไม่มีปุ่มทดสอบล็อกอิน (ห้ามแตะระบบประกันโดยไม่จำเป็น)",
      'id="testemcs"' not in _page and '/emcs-login-test' not in _webui_src)
...
check("+ ในความเห็นผู้ตรวจสอบ -> บวก",
      _PS("txtAcc_Comment", "รถประกันประเภท 2+ ตรวจแล้ว") == "รถประกันประเภท 2บวก ตรวจแล้ว")
check("+ ในรายละเอียดการเกิดเหตุ -> บวก",
      _PS("txtAcc_Detail", "คู่กรณีประเภท 2+") == "คู่กรณีประเภท 2บวก")
check("+ ในประเภทกรมธรรม์ (หน้าหลัก) -> บวก",
      _PS("txtPolicy_Type", "ประเภท 2+") == "ประเภท 2บวก")
# บล็อกคู่กรณีอยู่ใน naming container ของ ASP.NET - id จริงมี prefix นำหน้า
check("+ ในประเภทกรมธรรม์ของคู่กรณี (id มี prefix) -> บวก",
      _PS("wuOpoCar1_txtPolicy_Type", "ประเภท 2+") == "ประเภท 2บวก")
...
check("แปลงทุกจุดในข้อความเดียว",
      _PS("txtAcc_Comment", "2+ และ 3+") == "2บวก และ 3บวก")
...
check("+ หายตอนบันทึก = ไม่ตรง (ต้องจับได้)",
      not browser._cmp_value("ประเภท 2บวก", "ประเภท 2"))
...
browser._record_filled("txtAcc_Detail", "ชน ประเภท 2บวก")
...
check("EMCS กลืนคอมมา (เคส #446 21/09/69, v1.1.24): ตัด ',' ทิ้งก่อนกรอกในช่องชุด EATS (เว้นวรรคคงอยู่) → ตรวจกลับตรง · ช่องอื่นคงเดิม",
      browser._emcs_safe("txtAcc_result", "เป็นเงิน 7,000 บาท, ศุภชัย") == "เป็นเงิน 7000 บาท ศุภชัย"
      and browser._emcs_safe("txtDri_Address", "46/23, ม.7") == "46/23, ม.7")
...
check("ประเภทเคลมจากเว็บ: กลับด้านกับตัวแปลง ISURVEY→เว็บ ตรงกันทุกคู่ (ยกเว้น 04 เจรจาสินไหม ที่ยุบเข้า C ติดตาม)",
      all(_main._WEB_CLAIM_TYPE[web] == isv.lstrip("0")
          for isv, web in _conv.CLAIM_MTYPE_MAP.items() if isv != "04"))
# 15/09/69: 04 เจรจาสินไหม เป็นงานครั้งถัดไปเสมอ (ประเภทเคลมบน EMCS ใช้ของครั้งที่ 1) — เว็บโชว์เป็น "งานติดตาม"
# แทนปล่อยว่าง (user เลือกเองแบบนี้กับเคส #379) · ฝั่งบอทเข้า EMCS ตรง (CLAIM_TYPE_TO_EMCS["4"] is None) ไม่เปลี่ยน
check("ตัวแปลง ISURVEY→เว็บ: 04 เจรจาสินไหม → C งานติดตาม (ไม่ปล่อยว่าง)", _conv.CLAIM_MTYPE_MAP["04"] == "C")

# ---- 15/09/69 แบบ EMCS: งานครั้งที่ 2+ ตัวตรวจก่อนนำเข้าไม่กั้นด้วยด่านข้อมูลหลัก (มาจากครั้งที่ 1 บอทไม่แตะหน้าหลัก) ----
import inspect as _inspect_chk  # noqa: E402
_src_chk = _inspect_chk.getsource(_wui.check_sesurvey_case)
check("ตรวจก่อนนำเข้า: visit_no > 1 ย้ายด่านข้อมูลหลักไปเป็นคำเตือน เหลือกั้นแค่อ่านไฟล์/report ไม่ได้",
      "ด่านข้อมูลหลักไม่กั้น" in _src_chk and 'b.startswith("อ่านไฟล์ XML ไม่ได้")' in _src_chk and "blockers[:] = keep" in _src_chk)
...
check("ที่อยู่ผู้ขับขี่: หมู่ที่ปนในบ้านเลขที่แยกออกเป็น ม.<เลข> เสมอ · ช่องหมู่ที่ให้มาชนะ · ต. ย้ายไปท้าย · ว่างข้าม",
...
check("ประกอบผู้บาดเจ็บเคลม 2026013173082: ไม่ซ้ำอีก (เดิม '…อ.เมือง จันทบุรี อ.เมืองจันทบุรี')",
      ...
      and _dal("37 ต.หนองห้าง อ.อุทุมพรพิสัย ศรีสะเกษ", "7", "หนองห้าง") == "37 ม.7 ต.หนองห้าง อ.อุทุมพรพิสัย ศรีสะเกษ"   # มีแต่ช่องตำบล: ชื่อจังหวัดเปล่า ๆ ท้ายหางไม่กระโดดไปหัว (เคส #513)
      and _oal("37 ต.หนองห้าง อ.อุทุมพรพิสัย ศรีสะเกษ", "", "หนองห้าง", "", "") == "37 ต.หนองห้าง อ.อุทุมพรพิสัย ศรีสะเกษ")
