...
`listcases.php` มีข้อจำกัดที่ต้องรู้ (probe จริง 2026-08-04):

| | |
|---|---|
| `claim_status` | **ต้องส่งเสมอ** ไม่ส่ง = ได้ default 40 (รอตรวจข้อมูล) · **100 = จบงาน** |
| paging | **ใช้ไม่ได้** `start`/`page` เท่าไหร่ก็คืนชุดเดิม |
| `sort` | **ถูกเมิน** เรียงเก่า→ใหม่เสมอ (ไม่ใส่วันที่ = ได้งานปี 2020) |
| `claim_date` | "ตั้งแต่วันที่นี้เป็นต้นไป" · คืนสูงสุด **50 เรื่อง** |
| `total` | เท่ากับ limit เสมอ — **ไม่ใช่จำนวนจริง อย่าเชื่อ** |

→ อยากได้งานล่าสุดต้อง **ระบุวันที่เริ่ม** ไม่ใช่เลื่อนหน้า · ฝั่งเราเรียงตาม `close_datetime` ใหม่→เก่าเอง
⚠️ `close_datetime` ว่างในสถานะอื่น — **ห้ามใช้เป็นเกณฑ์ "จบงาน"** ให้ดู `sttcase_ID`
- หน้ารายการโชว์แค่ 15 ฟิลด์ที่ listcases ให้ — **ไม่ตรวจความครบตอนโหลด** (ยิง getcaseinfo
  ทีละเรื่อง 50 เรื่อง = ช้าและ ISURVEY timeout บ่อย) ตรวจตอนกดรายเรื่องแทน
