...
| ฝั่งกรอก EMCS **เคลมสด/คู่กรณี** | 🟢 คู่กรณีครบ 2026-06-18 (ดู §6.1): อ่าน Tab4 (combo) + กรอกฟิลด์/ประเภทรถ/ประกันประเภท/กรมธรรม์/ความเสียหาย + บันทึกผ่าน; เหลือ ยี่ห้อ/มีประกันภัยที่ เลือกเอง. ผู้บาดเจ็บ/ทรัพย์สิน (Tab5/6 กลไกเดียวกัน) ยังไม่เขียน |
...
- **รูปหลายประเภทในรอบเดียว (รูปรถประกัน + รูปรถคู่กรณี):** `upload_images` แยกเป็น
  ชุดต่อประเภท แล้วเรียก `_upload_one_type` (นำทาง `wuMenuPage1_imbImage` ใหม่ทุกชุด
  กัน stale หลังหน้า refresh) — เลือกประเภทใหม่ แล้วอัปทีละชุด
...
- **เปลี่ยนชื่อรูปคู่กรณีก่อนอัป** (`_rename_opponent_files`, เรียกใน
  ...
  **`รูปรถคู่กรณีคันที่<คัน>_<ลำดับ>.jpg`** (แพทเทิร์นเดียวกับรูปรถประกัน
  ...
  - `archive_old_images` ย้าย `tp_veh/` เข้า `_old/` ด้วย (รอบใหม่จะได้ไม่สะสม `_2`)
...
- **ที่เหลือ = ฝั่งกรอก EMCS:** (1) **probe field id แท็บ Injury Info + Asset Info** — ต้องใช้**เคลมสดที่ยังไม่ส่ง** (เคลมที่ส่งแล้ว view-mode ล็อก ห้ามแตะ); คาดปลดล็อกหลัง save หน้าหลักเหมือนคู่กรณี (2) เขียน `fill_injuries` + `fill_assets` (ลอกแพทเทิร์น `fill_third_parties`) (3) ทดสอบ `fill_third_parties` save จริง
