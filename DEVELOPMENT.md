...
| โหมดนำเข้า XML (`--import-xml` / แท็บ **นำเข้า XML(จบงาน)**) | 🔄 **24/09/69 (v1.1.38) ย้ายจากช่องติ๊กมาเป็นแท็บ + วิ่งเส้นเดียวกับแท็บ นำเข้า ISURVEY** (อ่าน API · ด่านสถานะจบงาน/ลำดับครั้ง/กันซ้ำ · หลายเคลมได้) ต่างแค่ขั้นสร้างเรื่อง: `main._prepare_xml_import` หา**รหัสบริษัทจาก prefix เลขเซอร์เวย์** (เดิมไม่ส่ง = ไอโออิทุกงาน) + เปิดหน้าเคลม ISURVEY กด 'ดาวน์โหลด XML' → `fill_imported`/`run_import` · ⏳ ยังไม่ได้ลองกับงานจริงหลังย้าย · เดิม: ✅ E2E verify 2026-06-24 (draft S68426066006) — EMCS import ฟอร์มหลักจาก SURV_REPORT XML (ปุ่ม imbFileImport_XML) → บอทอุดช่องว่าง/แก้ (cascade อำเภอ + เคลียร์เลขรับแจ้ง) → คู่กรณี/ความเสียหาย **free-text 20 ช่อง** (vs cmdNewReport 8) → ค่าใช้จ่าย. รองรับ >8 ดีขึ้น (ดู §6.4); ยังไม่ verify: ผู้บาดเจ็บ/ทรัพย์สิน, >8 เต็ม 20, รูป, กดส่งจริง |
...
6. **"แปลงค่า" บอททำได้ / "ตัดสินข้อเท็จจริง" ต้องให้คน** (กติกา user 2026-08-05)

   เส้นแบ่งไม่ใช่ของเก่า/ของใหม่ แต่คือ **ต้นทางมีคำตอบอยู่แล้วหรือเปล่า**

   | | ต้นทาง | บอททำอะไร |
   |---|---|---|
   | **แปลงค่า** | ISURVEY มีคำตอบ แต่คำ/รหัสไม่ตรงตัวเลือก EMCS | **แปลงให้เลย** ถ้าพิสูจน์การแปลงด้วย master ทั้งสองฝั่งแล้ว |
   | **ตัดสินข้อเท็จจริง** | ต้นทาง**ไม่มีช่องบอกความจริง**นั้นเลย | **ห้ามเดา** — เตือน + ย้อมเหลือง ให้คนเลือกบน EMCS |

   - แปลงค่า (ทำได้): คำนำหน้าผู้ขับขี่ ←เพศ/อายุ (§6.6) · ลักษณะความเสียหาย
     ←ลักษณะการเกิดเหตุ 58/58 (§6.7) · ประเภทเคลม 01/02/03 (§6.3.5)
   - ตัดสินข้อเท็จจริง (ห้ามเดา): งานนี้เป็น "งานนัดหมาย" หรือเปล่า — ISURVEY
     ไม่มีตัวเลือกนี้ในระบบ จึงไม่มีทางรู้จากข้อมูล ได้แค่ **สงสัยจากวันที่ที่ห่างกัน**
     → `appointment_hint()` เตือน ไม่เปลี่ยนค่า · ประเภทเคลม 04 เจรจาสินไหม
     (EMCS ไม่มีคู่ → ไม่ติ๊กให้)
   - **เกณฑ์ตัดสินว่าจะทำหรือจะเตือน: "พิสูจน์ยังไง"** ตอบไม่ได้ = เตือน
   - ⚠️ **เตือนต้องหายาก** ถ้าเตือน 10+ ข้อต่องาน คนจะเลิกอ่านทั้งกอง เกณฑ์:
     *"ถ้าคนเห็นข้อความนี้ เขาจะตัดสินใจต่างจากที่บอททำไหม"* — ถ้าไม่ ให้ทำแล้ว
     ย้อมเหลืองพอ (mark_check) ไม่ต้องหยุด ไม่ต้องเตือน
...
- **ช่องอายุถูก JS ของ EMCS เขียนทับ** (2026-09-02): `wuCale_Dri_BirthDay_txtCalendar` ผูก
  `onblur → getCalculate_Age(วันเกิด, txtDri_Age)` = คำนวณอายุจากวันเกิดเทียบ **วันนี้**
  (`new Date()`) แล้วเขียนทับช่องอายุ — บล็อกคู่กรณีทุกคัน (`dtlOpo_ctlNN_wuOpo_…`) ผูกแบบเดียวกัน
  ISURVEY ให้อายุ ณ วันเกิดเหตุ → ผู้ขับขี่ที่มีวันเกิดคั่นระหว่างวันเกิดเหตุกับวันคีย์จะต่างกัน 1 ปี
  (2026013166152: 53→54, 2026013167736: 45→46) → ตรวจกลับหลังบันทึกไม่ตรง → ประตูส่งงานปิด
  **ทางแก้** `_fill_age_after_birthdate`: กรอกวันเกิด → บังคับ focus+blur ให้ EMCS คำนวณของมันก่อน
  → ค่อยกรอกอายุจาก ISURVEY ทับ (ต่างกัน = log ℹ️ ให้คนตรวจรู้; อายุว่าง = ปล่อย EMCS คำนวณเอง)
  ใช้ทั้ง `fill_driver` และ `fill_third_parties`; smoke test ล็อกลำดับไว้
...
- **บอทกรอกหน้านี้มาก/น้อยตาม "ต้นทางข้อมูล" (กติกา user 2026-08-03)**
  ...
  | **se-survey** | `True` (ตั้งแต่ 2026-08-15) | เต็มหน้าเหมือน ISURVEY | เว็บ se-survey เป็นศูนย์กลาง — หัวหน้ากรอกยอด+ความเห็นบนเว็บก่อนอนุมัติแล้ว ยกมาได้ (กติกาเดิม 2026-08-03 กรอกแค่ 2 ช่องหัวบิล เพราะตอนนั้นหัวหน้ายังไม่กรอกบนเว็บ) |
...
- **กติกาจับคู่ข้อความ 4 ช่อง (user 07/09/69 — ยึดปุ่ม "นำเข้า ISURVEY" เป็นแม่แบบ ทั้งสองปุ่มต้องลง EMCS เหมือนกัน)**

  | ต้นทาง ISURVEY | → ช่อง EMCS | เส้น ISURVEY (`ClaimData`) | เส้น se-survey (ตัวแปลงตั้งค่าตั้งต้นบนเว็บ) |
  |---|---|---|---|
  | แท็บ 1 "บันทึกความเห็นหัวหน้างาน" (`accident_summary`) | **ผลการดำเนินงาน** `txtAcc_result` (หน้าค่าใช้จ่าย; หน้าแรกได้ฉบับยุบบรรทัด) | `accident_summary` | `survey_result` |
  | แท็บ 2 "ความคิดเห็นพนักงาน" (`surveyor_comment`) | **รายละเอียดการเกิดเหตุ** `txtAcc_Detail` (ข้อมูลทั่วไป) | `acc_detail` | `acc_detail` |
  | แท็บ 2 `acc_detail` ของ ISURVEY | **ไม่ใช้** — เป็นข้อความแม่แบบบริษัท (ติดต่อสาขา/ห้ามแนะนำอู่) + ข้อมูลกรมธรรม์ | – | – |
  | – | ความเห็นของผู้ตรวจสอบ `txtAcc_Comment` / ความเห็นของเซอร์เวย์ `txtSurv_Comment` | ว่าง | ว่างจากตัวแปลง (หัวหน้าพิมพ์บนเว็บได้ → ตามไปตามชื่อช่อง) |

  "ย้าย" ไม่ใช่ก๊อป — ข้อความเดียวกันต้องไม่โผล่ 2 ช่อง · ประวัติ: 13/08/69 เคยย้ายความเห็นหัวหน้าไป `txtAcc_Comment`
  (ยกเลิก 07/09/69) · verify ด้วยเคลม 2026013071573 ว่าสองปุ่มได้ข้อความเท่ากันทุกช่อง · โค้ด: `isurvey_api.read_claim`,
  `isurvey.read_tab1/2`, `isurvey_to_sesurvey.build_case`, `main._populate_claim_from_report` (จับคู่ตามชื่อ ไม่ต้องแก้)
  · ทางที่ 3 **อัปโหลดไฟล์ XML ของ ISURVEY บนเว็บ** — backend se-survey `xmlImport.service.ts` จับคู่แบบเดียวกัน
  (เฉพาะ source `isurvey_xml`; แต่ XML ของ ISURVEY ไม่มีบันทึกความเห็นหัวหน้างาน → ผลการดำเนินงานว่าง หัวหน้ากรอกบนเว็บ)
...
| บอทไม่กดส่งงานให้: `txtDri_Age` กรอก 53 แต่บนหน้าเป็น 54 (ตรวจกลับ 39/40) | onblur ช่องวันเกิดของ EMCS เรียก `getCalculate_Age` เขียนอายุ "ณ วันนี้" ทับ — บอทกรอกอายุ (ISURVEY = ณ วันเกิดเหตุ) ก่อนวันเกิด จึงถูกทับตอน focus ออกจากช่องวันเกิด; เกิดเฉพาะผู้ขับขี่ที่วันเกิดคั่นระหว่างวันเกิดเหตุกับวันคีย์ (2026013166152, 2026013167736) | `_fill_age_after_birthdate`: วันเกิดก่อน → บังคับ blur → อายุทีหลัง ทั้งผู้ขับขี่ประกัน + คู่กรณีทุกคัน (ดู §4 ฟอร์มหลัก) |
...
- ~~**ประเภทเคลม = ISURVEY type-1 ตรง ๆ**~~ ❌ **ไม่จริง — แก้แล้ว 2026-08-05**
  ดัมป์ master ทั้งสองฝั่งมาเทียบ แล้วพบว่า **ลำดับไม่ตรงกัน**:

  | # | ISURVEY `masterClaimMType` | EMCS `rdoSurv_Claim_Type` |
  |---|---|---|
  | 1 | เคลมสด | `[0]` เคลมสด ✅ |
  | 2 | เคลมแห้ง | `[1]` เคลมแห้ง ✅ |
  | 3 | **ติดตาม** | `[2]` **งานนัดหมาย** ❌ |
  | 4 | **เจรจาสินไหม** | `[3]` **งานติดตาม** ❌ |

  เดิม `fill_claim_type` แปลงด้วย index ตรง ๆ (`idx = ประเภท - 1`) → ถูกเฉพาะ 01/02
  ถ้าเจอ 03 จะไปติ๊ก "งานนัดหมาย" และ 04 เป็น "งานติดตาม" **ผิดทั้งคู่แบบเงียบ ๆ**
  (ยังไม่เคยปะทุเพราะงานที่ทำมาเป็น 01/02 หมด) → ใช้ `CLAIM_TYPE_TO_EMCS` แปลงตาม
  **ความหมาย** แทน; `04 เจรจาสินไหม` = EMCS ไม่มีคู่ → ไม่ติ๊กให้ ให้คนเลือกเอง
  ⚠️ **"งานนัดหมาย" ของ EMCS ไม่มีต้นทางใน ISURVEY เลย** — เลือกได้เฉพาะคนบนหน้าจอ
- **เคลมที่ควรเป็น "งานนัดหมาย" จะติดด่าน 24 ชม. ของ EMCS** (เจอจริง 2026013058422:
  เกิดเหตุ 25/05 แต่ออกตรวจตามนัด 20/07 = ห่าง 56 วัน) เพราะ ISURVEY ไม่มีตัวเลือก
  นัดหมาย ต้นทางจึงติดเป็น 'เคลมสด' มาเรื่อย ๆ → `appointment_hint()` ตรวจให้แล้วเตือน
  **2 ที่**: แผง 🔍 ตรวจ (ก่อนเปิด EMCS) และตอนกรอกจริง (ย้อมเหลือง radio ทั้งชุด)
  — **แจ้งเตือนอย่างเดียว ไม่เปลี่ยนค่าให้** (กติกา user: ให้คนเลือกบน EMCS)
- ด่านเคลมแห้งเช็ค `claim_type == "2"` ตรง ๆ (ไม่พึ่งข้อมูลคู่กรณีอย่างเดียว
  เพราะ XML โหลดพลาดได้)
...
## กดบันทึกหน้าหลักแล้ว "เงียบ" — หาต้นเหตุเจอแล้ว (03/09/69)

**อาการ:** คลิกปุ่มบันทึกหน้าหลัก แล้ว EMCS ไม่ตอบอะไรเลย — ไม่มี alert ไม่มีการบันทึก
ปุ่มกลับเป็น 'แก้ไข' · กดใหม่ 2 วินาทีให้หลังด้วยข้อมูลชุดเดิม **ผ่านเสมอ** · เกิด ~2 ใน 3 ครั้ง
ช่วงบ่าย (ทดสอบซ้ำกับ draft S68426090922 ของเคส #198 กว่า 25 รอบ)

**ต้นเหตุ (จากการดัมพ์สภาพหน้า ณ วินาทีที่เงียบ ไม่ใช่การเดา):**
หน้า EMCS เป็น **ASP.NET AJAX — UpdatePanel 130 ใบ** และไม่ประกาศ control ไหนเป็น async/full
→ dropdown (จังหวัด/อำเภอ/ประเภทรถ ฯลฯ) ยิง postback แบบ **XHR patch หน้าอยู่กับที่**
(เอกสารไม่ถูกแทนที่ marker บน window ไม่หาย) · พอกดปุ่มบันทึกซึ่งเป็น **full postback ครั้งแรก**
หลังจากนั้น เซิร์ฟเวอร์บางครั้ง**ไม่ยิง event ของปุ่ม** แค่ render หน้าใหม่ — หลักฐาน:
`nav.type=navigate` เกิด ~2-3 วิหลังคลิก · ไม่มี alert · `inAsync=false` · ปุ่ม 'แก้ไข' enabled
· hidden field ครบ (`__EVENTTARGET` อยู่ใน form) · validForm ผ่าน · **รอ 25 วิก่อนกดก็ยังเกิด**
= ไม่ใช่เรื่องจังหวะ/postback ค้าง แต่เป็นสถานะหน้าหลังโดน XHR patch หลายรอบ
คลิกที่สองลงบนหน้าที่เพิ่ง render ใหม่ (สะอาด) จึงผ่านทุกครั้ง

**สิ่งที่ทำ (ทั้งหมดอยู่ใน `_click_save_button` / `save_main_form` / `browser.py`):**
1. `wait_postback_done` + `arm_submit_watch` — ดักตอนฟอร์ม "สั่ง submit จริง" (ห่อ `__doPostBack`/
   `form.submit` + ฟัง `beginRequest`/`endRequest` ของ PageRequestManager) ก่อนกดบันทึก/
   ก่อนเลือก dropdown ถัดไป ถ้ามี postback ค้างจริงจึงรอ — แทนการเดาเวลานิ่ง
   ⛔ ต้องรู้จัก AJAX: รุ่นแรกรอ "เอกสารถูกแทนที่" อย่างเดียว เลยรอจนหมดเวลาทุก dropdown
2. `_wait_alert_or_refresh` — หลังคลิก ถ้าเอกสารถูกแทนที่แล้ว (ตัวดักคลิกหาย) โหลดเสร็จ 2 วิ
   ยังไม่มี alert = "refresh ไม่ยิง event" → **กดใหม่ทันที** (เสีย ~4 วิ แทน 9-33 วิ)
3. เรียงลำดับกรอกตาม DOM จริง · blur ก่อนกด · ข้าม dropdown ที่ค่าตรงอยู่แล้ว (รวมที่ต่างแค่
   การสะกดของ EMCS เช่น 'ส่วนบุคคคล') — ช่วยลด postback ที่ไม่จำเป็น แต่**ไม่ใช่**ตัวแก้อาการนี้

**ทฤษฎีที่พิสูจน์แล้วว่าผิด (อย่าไล่ซ้ำ):** postback ของ dropdown ค้างตอนกด (รอ 25 วิก็ยังเกิด) ·
โฟกัสค้างในช่องสุดท้าย · hidden field `__EVENTTARGET` หาย (ดัมพ์แล้วมีครบ) · marker บน window
หลังคลิก (หายจาก XHR ได้) · ASP.NET validator (หน้าไม่มี) · การอ่านหน้าก่อนคลิก (A/B แล้วไม่ต่าง)

**ผลหลังแก้:** 3 รอบท้าย (18:06-18:08) ผ่านคลิกแรกทั้งหมด 3-4 วิ · งานทั้งใบ 47-48 วิ (ก่อนแก้ 4 นาที) · ตรวจกลับ 34/34 + 5/5 ทุกรอบ · อาการ 'refresh ไม่ยิง event' ยังเกิดได้เป็นครั้งคราว (เป็นของฝั่ง EMCS) แต่ราคาลดเหลือ ~4 วิ และกู้เองเสมอ — เดิม 33 วิ/ครั้ง

**ของแถมที่เจอระหว่างทาง**
- เปิด draft เดิมมากรอกทับ: EMCS **ไม่ render ค่าจังหวัด/อำเภอกลับมา** (ขึ้น placeholder ทั้งที่
  บันทึกไว้แล้ว) → ต้องเลือกใหม่ทุกครั้ง ห้ามข้าม ไม่งั้นค่าหายจาก draft
- ตัวเลือกประเภทใบขับขี่ของ EMCS สะกด `ใบขับขี่รถยนต์ส่วนบุคคคล` (ค 3 ตัว) → ข้ามให้แล้วเมื่อ
  คะแนนความเหมือน ≥97
- EMCS กลืนเครื่องหมาย `*` (เข้าชุดเดียวกับ `+` `%` em-dash) — `***วุฒิชัย***` เหลือ `วุฒิชัย`
- "การเรียกร้องค่าเสียหายจากคู่กรณี" + "คู่กรณีคันที่" บังคับ**เฉพาะ**ผลคดี = รถคู่กรณีเป็นฝ่ายผิด
  (อ่านจาก `validForm()` ตรง ๆ) — เตือนนอกเงื่อนไขนี้คือเตือนหลอก

## audit ตัวแปลง ISURVEY → se-survey ทั้งตัว (03/09/69)

`python tools/audit_isurvey_converter.py --claim <เลขเคลม> --survey <เลขเซอร์เวย์>` — อ่าน ISURVEY อย่างเดียว
ดัมป์ข้อมูลดิบไว้ `runs/audit/` แล้วเทียบผลของ `build_case()` ทีละช่องกับ **ชุดค่าที่เว็บ se-survey รับจริง**
(อ่านจากซอร์ส caseOptions/CaseDetail/RecordEditors/districtOptions/xmlExport) · `--raw` เล่นซ้ำจากไฟล์ดิบได้
· เทสสัญญา `tests/test_isurvey_to_sesurvey.py`

ตรวจ 3 เคลมจริง (มีคู่กรณี/ผู้บาดเจ็บ/ทรัพย์สิน) เจอและแก้แล้ว:
- ความสัมพันธ์: API ส่ง **คำ** ('ลูกจ้าง') ไม่ใช่รหัส และ masterRelation ว่าง → RELATION_MAP (รหัส) ไม่เคยจับ → `_relation()` รับคำใน 40 ค่าของ EMCS
- บริษัทประกันคู่กรณี: `oth_insure_company_name` เป็น None ทุกเคส มีแต่รหัส → resolve ผ่าน `api._company()`
- ทรัพย์สิน (tab-6): record ห่อใต้คีย์ `property` → อ่านชั้นบนได้ว่างหมด → `_flat()` ก่อน (เหมือน tab-5 `patient`)
- ชิ้นส่วน: ISURVEY ใส่ข้างท้ายชื่อ ('บังโคลนหน้าขวา') → `split_part_side()` แยกเป็น pos L/R/A ให้ตรง checklist EMCS
- จำนวนครั้ง/จำนวนรูปเป็น 0 ทั้งที่มียอด (ส่วนขยาย se-billing กรอกแต่ยอด) → ค่าบริการ/เดินทาง = 1 ครั้ง · ค่ารูป = ยอด ÷ 5
- ชื่อที่มีอักขระ EMCS ไม่รับ (วงเล็บ) → `_name()` ล้างตั้งแต่ตอนดึง · บัตรไม่ใช่ 13 หลัก → `driver_id_type=foreign`
