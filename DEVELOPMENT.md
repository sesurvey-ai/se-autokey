...
- **ผู้บาดเจ็บ:** เมนู `wuMenuPage1_imbInjure_Person` → `ddlInj_Count` (1-5) →
  ...
  - **⚠️ `txtCar_RegNo` (เลขทะเบียน) บังคับก่อนเข้าหน้าค่าใช้จ่าย** แต่ ISURVEY ว่าง:
    กดเมนูค่าใช้จ่ายจะเด้ง alert "ไม่สามารถไปหน้า [ค่าใช้จ่าย] ได้ กรุณาตรวจสอบ ... [ผู้บาดเจ็บ]
    ... เลขทะเบียน คนที่ N" → **ผู้ใช้กรอกบน webui** (ดู §5): `fill_injuries` เรียก
    `wait_for_injury_inputs` ก่อน — webui โชว์ฟอร์มต่อคน (dropdown ประเภท default จาก
    ISURVEY แก้ได้ + ช่องเลขทะเบียน) ค่าที่ได้ override person_type/car_regno;
    console/EOF = ใช้ค่า ISURVEY (เลขทะเบียนว่าง → fill_billing จับ alert + หยุดรอ)
