...
## กดบันทึกหน้าหลักแล้ว "เงียบ" — สรุปการไล่หาต้นเหตุ (03/09/69)

**อาการ:** คลิกปุ่มบันทึกหน้าหลัก (`btnUpdate`/`btnSave`) แล้ว EMCS ไม่ตอบอะไรเลย —
ไม่มี alert ไม่มี postback ไม่มี error · กดใหม่อีก 2 วินาทีให้หลังด้วย **ข้อมูลชุดเดิมเป๊ะ**
ผ่านทันที · เกิดกับทั้งเคสใหม่และโหมดกรอกทับ draft เดิม

**สถิติจากเคส #198 (S68426090922) 8 รอบทดสอบ:** คลิกแรกหาย ~2 ใน 3 ครั้ง
· คลิกที่สองผ่านเสมอภายใน 3-5 วิ · ไม่เคยเห็นคลิกที่สามเลย

**ไล่หาต้นเหตุแล้ว 4 ทาง — ช่วยให้เกิดน้อยลง แต่ไม่มีทางไหนทำให้หายขาด**

| สมมติฐาน | ทำอะไร | ผล |
|---|---|---|
| ลำดับกรอกทำให้ postback ค้างตอนกด | เรียงการกรอกใหม่ตาม DOM จริง (dropdown ไปอยู่ต้นบล็อก เหลือช่องข้อความ ~30 ช่องคั่นก่อนกดบันทึก) | ดีขึ้นชัด แต่ยังหลุด |
| โฟกัสค้างในช่องสุดท้าย → onblur ยิง postback ตอนคลิก | `blur()` เองก่อนรอหน้านิ่ง | ยังหลุด (เก็บไว้ ไม่มีผลเสีย) |
| postback ของ dropdown ยังไม่จบตอนกด | `_settle_after_select()` รอ marker จนนิ่งหลังเลือกทุกครั้ง | ยังหลุด (เก็บไว้ — แก้ปัญหาค่าหายจาก render ทับด้วย) |
| แยก "ฟอร์มถูกส่ง" ออกจาก "onclick ทำงาน" ด้วย marker บน window | `_postback_started()` | **ใช้ไม่ได้ — ถอดออกแล้ว** marker หายจาก postback ตัวอื่นได้เหมือนกัน จึงคืน "ส่งแล้ว" ทั้งที่ไม่ได้ส่ง |

**ที่ทำแทน (ยอมรับพฤติกรรม แล้วทำให้ราคาถูกที่สุด):** รอ alert รอบแรก ๆ แค่ 4 วิ
แล้วกดใหม่ทันที รอบสุดท้ายรอเต็ม 12 วิ → คลิกหาย 1 ครั้งเสีย ~9 วิ (เดิม 33 วิ)

**ตัวเลขก่อน/หลังของจังหวะกดบันทึกหน้าหลัก (เคสเดียวกัน):**
101 วิ (3 รอบ) → 4 วิ ถ้าไม่หลุด · ~15 วิ ถ้าหลุด 1 ครั้ง

⛔ อย่าเสียเวลาไล่ซ้ำทางเดิม — ถ้าจะไล่ต่อ ให้เริ่มจากอ่าน `window.__seClick` (ตัวดัก
ที่ `_arm_click_probe` ติดไว้) ตอนคลิกหาย ว่า `got`/`ran`/`ret` เป็นอะไร แล้วเทียบกับ
รอบที่ผ่าน — ยังไม่เคยเก็บข้อมูลชุดนี้มาเทียบกันจริงจัง

**ของแถมที่เจอระหว่างทาง**
- เปิด draft เดิมมากรอกทับ: EMCS **ไม่ render ค่าจังหวัด/อำเภอกลับมา** (ขึ้น `-- จังหวัด --`
  ทั้งที่บันทึกไว้แล้ว) → ต้องเลือกใหม่ทุกครั้ง ห้ามข้าม ไม่งั้นค่าหายจาก draft
- ตัวเลือกประเภทใบขับขี่ของ EMCS สะกด `ใบขับขี่รถยนต์ส่วนบุคคคล` (ค 3 ตัว) — ต่างจากต้นทาง
  1 ตัวอักษร ทำให้เลือกซ้ำฟรีทุกรอบ · ตอนนี้ข้ามให้แล้วถ้าคะแนนความเหมือน ≥97
- EMCS กลืนเครื่องหมาย `*` (เพิ่มเข้าชุดเดียวกับ `+` `%` em-dash) — `***วุฒิชัย***` เหลือ `วุฒิชัย`
- "การเรียกร้องค่าเสียหายจากคู่กรณี" + "คู่กรณีคันที่" บังคับ**เฉพาะ**ตอนผลคดี =
  รถคู่กรณีเป็นฝ่ายผิด (อ่านจาก `validForm()` ของ EMCS ตรง ๆ) — เตือนนอกเงื่อนไขนี้ = เตือนหลอก
