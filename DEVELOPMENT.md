...
- **ผู้บาดเจ็บ:** เมนู `wuMenuPage1_imbInjure_Person` → `ddlInj_Count` (1-5) →
  ...
  - **⚠️ `txtCar_RegNo` (เลขทะเบียน) บังคับก่อนเข้าหน้าค่าใช้จ่าย** แต่ ISURVEY ว่าง:
    ...
    `wait_for_injury_inputs` **หลังเลือกจำนวน** (เพื่ออ่าน options dynamic) — webui โชว์
    ฟอร์มต่อคน (dropdown ประเภท default smart + ช่องเลขทะเบียน) ค่าที่ได้ override
    person_type/car_regno;
