...
def name_or_unknown(v) -> str:
    """ชื่อผู้ขับขี่คู่กรณี / ผู้บาดเจ็บ / เจ้าของทรัพย์สิน ที่ไม่ทราบ (ว่าง · "รอตรวจสอบ" · ขีดกี่ขีดก็ตาม) → "ไม่ทราบชื่อ"
    (user เคาะ 21/09/69 เคส #433 — เดิม "-") ⛔ ไม่ใช้กับผู้ขับขี่รถประกัน/เจ้าของรถคู่กรณี · เรียกหลัง with_title"""
    s = str(v or "").strip()
    return UNKNOWN_NAME if (not s or is_placeholder(s)) else s


def birth_placeholder_if_this_year(bd) -> str:
    """วันเกิดที่อยู่ในปีปัจจุบันหรืออนาคต (01/01/2569 ที่คนพิมพ์แทน "ไม่ทราบ" — เคส #433) = ไม่ใช่วันเกิดจริง
    → วันเกิดตัวแทนค่า "01/01/2500" (อายุคำนวณ 69) · อื่น ๆ คืนค่าเดิม (user เคาะ 21/09/69)"""
    from datetime import date
    s = str(bd or "").strip()
    p = parse_real_date(s)
    return "01/01/2500" if (p and p[0] >= date.today().year) else s
