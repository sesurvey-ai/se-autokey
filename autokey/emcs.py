...
#   'ประกันตรวจสอบงานต่อเนื่อง' = กด 'ส่งผลงานต่อเนื่อง' (ครั้งที่ 2+) แล้ว รอประกันตรวจ — เห็นจริง 22/09/69 เคลม 2025013053652
#   (สถานะเป็นของทั้งเรื่อง: ครั้งก่อนหน้าต้องส่งแล้วถึงเปิดครั้งถัดไปได้ → ครั้งล่าสุดส่งแล้วแน่)
#   'ตรวจสอบแล้ว' = งานเสร็จสมบูรณ์ — ฝั่งประกันตรวจงาน + อนุมัติเงินแล้ว (หลัง 'ประกันตรวจสอบรายงาน') — user ยืนยัน 22/09/69
#   ลำดับ: รายงานสร้างใหม่ (draft) → ประกันตรวจสอบรายงาน / ประกันตรวจสอบงานต่อเนื่อง (ส่งแล้ว รอประกัน) → ตรวจสอบแล้ว (จบ)
SUBMITTED_STATUSES = {"ประกันตรวจสอบรายงาน", "ประกันตรวจสอบงานต่อเนื่อง", "ตรวจสอบแล้ว"}
COMPLETED_STATUS = "ตรวจสอบแล้ว"
...
def is_report_submitted(driver, claim: str, survey_no: str = ""):
    ...
    if not info and survey_no:
        # งานต่อเนื่อง (ครั้งที่ 2+): แถวในหน้ารายการโชว์เลขครั้งแรก เลขครั้งถัดไปหาไม่เจอ → ถ้าเคลมมีเรื่องเดียว บอกสถานะเรื่อง
        # แต่ **ไม่สรุปว่าครั้งนี้ส่งแล้ว** (สถานะบนแถวเป็นของทั้งเรื่อง แยกครั้งไม่ได้) — รอบกวาดจะได้ข้อความที่ตรงความจริง (22/09/69)
        one = report_status(driver, claim)
        if one:
            st1 = (one.get("status") or "").strip() or "อ่านไม่ได้"
            if st1 == "ประกันตรวจสอบงานต่อเนื่อง":
                # สถานะนี้ขึ้นเฉพาะหลังกด 'ส่งผลงานต่อเนื่อง' — ครั้งล่าสุดของเรื่องส่งแล้วแน่ (ครั้งก่อนต้องส่งก่อนถึงเปิดครั้งถัดไปได้)
                return True, f"ส่งผลงานต่อเนื่องแล้ว (สถานะ: {st1} · เรื่อง {one.get('esurvey') or '-'})"
            if st1 == COMPLETED_STATUS:
                # ทั้งเรื่องจบสมบูรณ์ (ประกันตรวจ+อนุมัติเงินแล้ว) → ทุกครั้งในเรื่องส่งแล้ว
                return True, f"ส่งงานแล้ว ประกันตรวจสอบและอนุมัติเงินแล้ว (สถานะ: {st1} · เรื่อง {one.get('esurvey') or '-'})"
            return None, (f"งานต่อเนื่อง: เลข {survey_no} ไม่อยู่ในแถว (เรื่อง {one.get('esurvey') or '-'} สถานะ {st1}) "
                          "— ครั้งนี้ยืนยันจากหน้ารายการไม่ได้")
    if not info:
        return None, "ไม่พบเรื่องของเคลมนี้ใน EMCS (หรือแยกเรื่องไม่ออก)"
    st = (info.get("status") or "").strip()
    if not st:
        return None, "อ่านสถานะเรื่องไม่ได้"
    if st == COMPLETED_STATUS:
        return True, f"ส่งงานแล้ว ประกันตรวจสอบและอนุมัติเงินแล้ว (สถานะ: {st})"
