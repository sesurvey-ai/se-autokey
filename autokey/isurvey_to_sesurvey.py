...
#: 04 'เจรจาสินไหม' ไม่มีคู่ตรงในฝั่งเรา/EMCS (เป็นงานครั้งถัดไปเสมอ ประเภทเคลมบน EMCS ใช้ของครั้งที่ 1) →
#: 15/09/69 แมปเป็น C 'งานติดตาม' (user เลือกเองแบบนี้กับเคส #379 เคลม 2026013057520) แทนปล่อยว่างแล้วเตือน
CLAIM_MTYPE_MAP = {"01": "F", "02": "D", "03": "C", "04": "C"}
...
def build_case(api, case_id: str, listrow: dict | None = None) -> dict:
    ...
    report.update({
        ...
        # ชนิดบัตรให้ "คน" เลือกบนเว็บ ไม่คำนวณจากเลข (user เคาะ 21/09/69) — ค่าเริ่มต้นคนไทย: ไม่ครบ 13/มีตัวอักษร เว็บจะเตือนแดง หัวหน้าสลับเป็นต่างชาติเอง
        "driver_id_type": "thai",
    })
    if not veh:
...
def _injuries(api, case_id, warnings: list) -> list:
    ...
    for row in api.list_records(case_id, 5):
        ...
        out.append({
            ...
            "age": _s(r.get("age")),
            "cid": icid,
            "id_type": "thai",   # ชนิดบัตรให้คนเลือก ไม่คำนวณจากเลข (21/09/69)
